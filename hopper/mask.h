...
struct Mask {
    ...
    Mask(const int thread_idx, const int seqlen_q, const int seqlen_k,
         ...
         cutlass::FastDivmod const &attention_chunk_divmod,
         cutlass::FastDivmod const &qhead_per_khead_divmod,
         int const * image_token_end = nullptr, int offset_q = 0)
            ...
            if constexpr (!SwapAB) {
                ...
                if constexpr (Causal_mask) {
                    ...
                    for (int m = 0; m < size<0>(tSrS_rowcol); ++m) {
                        ...
                        int const causal_col_limit_right = !Seqlenk_mask
                            ? row_idx + causal_row_offset
                            : __viaddmin_s32(row_idx, causal_row_offset, seqlenk_col_limit);
                        int const image_col_limit_right = image_token_end != nullptr && row_idx < seqlen_q
                            ? image_token_end[offset_q + row_idx] - n_block * kBlockN - thread_col_offset
                            : 0;
                        int const col_limit_right = std::min(
                            std::max(causal_col_limit_right, image_col_limit_right),
                            seqlenk_col_limit);
                        #pragma unroll
                        for (int n = 0; n < size<1>(tSrS_rowcol); ++n) {
